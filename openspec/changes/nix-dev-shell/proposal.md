...
- **Node 26 becomes the documented version everywhere.** `.nvmrc` becomes `26`. `AGENTS.md`, `README.md`, `backend/README.md` and `CONTRIBUTING.md` all say Node 26, and every CI workflow reads `.nvmrc` directly instead of hardcoding its own copy of the version.
...
- **Changed files:**
  ...
  - every `.github/workflows/*.yml` file that names a Node version — `ci.yml`, `docker-build-test.yml`, `docker-build-push.yml` and `release.yml` — each now reads `.nvmrc` through `actions/setup-node`'s `node-version-file` input, instead of hardcoding its own copy. (An earlier pass touched only `ci.yml`, then hardcoded the other three too; an external review pointed at `node-version-file` as the better fix.)
...
- **Not affected:**
  - the existing CI workflows, beyond how each one selects its Node version;
